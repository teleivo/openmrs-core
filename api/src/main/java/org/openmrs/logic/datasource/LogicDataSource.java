 ...
 * </ul>
 * A consumer of the logic service could then use <pre>
 * Context.getLogicService().eval(myPatient, &quot;@pharmacy visit.pharmacy&quot;);
 * </pre> to get a list of the names of pharamacies visited by the patient. However, this limits the
 * ways we can use the result. On the other hand, consider defining the key:
 * <ul>
 ...
 * </ul>
 * Now a consumer of the logic service can use the same result in several different ways: <pre>
 * 
 ...
 * </pre> One way to approach the design of a new data service is to avoid thinking of the
 * individual attributes, but rather think about turning the data into the fewest number of
 * observations (a key for each) filled with as much data as possible. Remember that you can
 * overload values within results; however, such overloading should aim to be as intuitive as
 * possible and well documented. New logic data sources should be documented on the
 * <a href="http://openmrs.org/wiki/Logic_Data_Sources">OpenMRS wiki</a>, including a description of
